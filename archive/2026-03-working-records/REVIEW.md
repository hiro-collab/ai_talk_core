...
- Medium: `--mic-loop` 開始時の `[mic-tuning] ...` 表示は有用ですが、`--command-only` でも stdout に必ず出るため、後段が標準出力をそのまま instruction として読む運用ではノイズになります。対象: [src/main.py](<workspace>/ai-talk-core/src/main.py)
- Low: `--list-mic-profiles` と `[mic-tuning] ...` の追加自体は妥当です。profile ごとの VAD / final しきい値が見えるので、調整結果を把握しやすくなっています。対象: [src/main.py](<workspace>/ai-talk-core/src/main.py), [README.md](<workspace>/ai-talk-core/README.md)
...
- Low: `--show-runtime-status` と `get_runtime_status()` の追加は妥当です。`transcription_device` と `runtime_note` が入り、GPU 不可時の CPU fallback を CLI から読み取りやすくなっています。対象: [src/io/audio.py](<workspace>/ai-talk-core/src/io/audio.py), [src/main.py](<workspace>/ai-talk-core/src/main.py)
- Low: 前回の懸念だった `[mic-tuning] ...` による stdout 汚染は改善されています。`[mic-tuning] ...` と `Stopped microphone loop.` を stderr に逃がしたことで、`--instruction-only` や handoff 用の stdout を壊しにくくなりました。対象: [src/main.py](<workspace>/ai-talk-core/src/main.py)
- Low: 記録側では [SHARE_NOTE.md](<workspace>/ai-talk-core/SHARE_NOTE.md) の `latest reviewed commit` がまだ `fd7e728` のままで、実装の現在地より古いです。今回の runtime status 追加自体は反映されていますが、この欄だけは stale です。
...
- Low: 今回の差分にはコード本体の変更がありません。確認できたのは [MEMORY.md](<workspace>/ai-talk-core/MEMORY.md) の `Codex startup` 追記と、未追跡の [AGENTS.md](<workspace>/ai-talk-core/AGENTS.md) だけでした。そのため、コードレビューとしての新しいバグ・設計所見はありません。
- Low: `Codex startup` の内容は運用メモとしては有用ですが、短期的な起動手順まで [MEMORY.md](<workspace>/ai-talk-core/MEMORY.md) に入れると、長期ルールと日次運用の境界が少し曖昧になります。恒常的な repository guidance は [AGENTS.md](<workspace>/ai-talk-core/AGENTS.md) 側、長期前提は [MEMORY.md](<workspace>/ai-talk-core/MEMORY.md) 側、と分けたままの方が整理しやすいです。
...
- Low: `--doctor` の追加は妥当です。runtime と dependency の状態を 1 回で見られるので、今回の Torch/CUDA 不整合のような切り分けには有効です。対象: [src/main.py](<workspace>/ai-talk-core/src/main.py)
- Low: 実装自体に大きな問題は見当たりませんが、記録同期は少し遅れています。[SHARE_NOTE.md](<workspace>/ai-talk-core/SHARE_NOTE.md) の current status はまだ `90 件` の smoke test 成功のままで、今回確認した `97 tests OK` に追いついていません。[LOG.md](<workspace>/ai-talk-core/LOG.md) も同様です。
- Low: [MEMORY.md](<workspace>/ai-talk-core/MEMORY.md) の `Codex startup` と未追跡の [AGENTS.md](<workspace>/ai-talk-core/AGENTS.md) が併存しており、運用メモの置き場が少し重なっています。コードの不具合ではありませんが、役割分担としては整理余地があります。
...
- Medium: プロジェクトの主目的が「Whisper による最小文字起こし」ではなく「音声で拾った指示を agent に handoff すること」であるのに、[README.md](<workspace>/ai-talk-core/README.md) の冒頭と構成はまだ転写ツール中心に見える。特に [README.md](<workspace>/ai-talk-core/README.md#L3) の導入、[README.md](<workspace>/ai-talk-core/README.md#L8) の Overview、[README.md](<workspace>/ai-talk-core/README.md#L168) の互換説明が同じ重さで並んでおり、初見では「何のためのプロジェクトか」が掴みにくい。
- Medium: `agent_*` へ寄せる方針に対して、互換用の `codex_*` 導線が README とテストで前に出すぎている。実装上の互換ラッパーは小さいが、[src/agent_runner.py](<workspace>/ai-talk-core/src/agent_runner.py)、[src/codex_runner.py](<workspace>/ai-talk-core/src/codex_runner.py)、[src/agent_handoff.py](<workspace>/ai-talk-core/src/agent_handoff.py)、[src/codex_handoff.py](<workspace>/ai-talk-core/src/codex_handoff.py) の二系統が README と `smoke_test.py` 上でも並列に見え、読者に不要な選択肢を増やしている。
- Medium: [src/main.py](<workspace>/ai-talk-core/src/main.py) は CLI 本体に加えて、mic-loop 調整、handoff 保存、runtime/dependency doctor まで抱えており責務が重い。特に [src/main.py](<workspace>/ai-talk-core/src/main.py#L339) 以降の parser と [src/main.py](<workspace>/ai-talk-core/src/main.py#L492) 以降の分岐は、通常利用の入口としても保守観点でも読み負荷が高い。
- Medium: [src/web/app.py](<workspace>/ai-talk-core/src/web/app.py) は将来もメンテナンス用ツールとして残す前提に対し、UI テンプレート、フロント JS、リクエスト処理、転写サービス呼び出しが 1 ファイルに集中している。特に [src/web/app.py](<workspace>/ai-talk-core/src/web/app.py#L582) の `process_transcription_request()` は flag 解釈、一時ファイル処理、正規化、転写、handoff 保存、レスポンス整形まで一括で、保守用 UI として長く持つには見通しが悪い。
- Low-Medium: [smoke_test.py](<workspace>/ai-talk-core/smoke_test.py) は CLI / Web / API / handoff / runner / finalization を 1 ファイルに集約しており、カバレッジ自体は広いが、読みやすさと変更追跡性は落ちている。特に [smoke_test.py](<workspace>/ai-talk-core/smoke_test.py#L71) の互換 alias 群を含め、「何を守るテストか」の輪郭が掴みにくい。
- Low: README の説明には現状との差が残っている。[README.md](<workspace>/ai-talk-core/README.md#L9) では `VAD` を「まだできないこと」に入れている一方、[README.md](<workspace>/ai-talk-core/README.md#L52) では `webrtcvad speech detection` を mic-loop flow に含めており、実装にも VAD 関連の入口がある。[src/main.py](<workspace>/ai-talk-core/src/main.py#L33) これは「本格 VAD は未完成」なのか「軽量 VAD は導入済み」なのかを README 上で書き分けた方がよい。
- Low: 細部の可読性では、[src/main.py](<workspace>/ai-talk-core/src/main.py#L49) の `format_transcription_result()` が `object` を受けて `getattr` ベースになっている点や、[src/io/audio.py](<workspace>/ai-talk-core/src/io/audio.py#L113) 付近の空行の乱れなど、小さいが積み重なる読みにくさがある。大きな不具合ではないが、整理局面では拾いたい。
...
- 1. [README.md](<workspace>/ai-talk-core/README.md) の冒頭を `agent handoff` 主体に書き換え、`transcribe` は手段、`Web UI` は保守用導線と分かる順序へ再構成する。
- 2. README と CLI 例は `agent_*` を主導線に固定し、`codex_*` は互換セクションへ退避する。
- 3. [src/web/app.py](<workspace>/ai-talk-core/src/web/app.py) から転写処理本体を service 関数へ切り出し、テンプレートとリクエスト処理の責務を分ける。
- 4. [src/main.py](<workspace>/ai-talk-core/src/main.py) の doctor/runtime/dependency 系と mic-loop 系の責務を段階的に分離する。
- 5. [smoke_test.py](<workspace>/ai-talk-core/smoke_test.py) を少なくとも `CLI / Web / Handoff` の論理単位に整理し、レビュー時に意図を追いやすくする。

### README restructuring note

- README の再構成自体は、コードや互換導線との整合を見ながら進める必要があるため、実装担当が [README.md](<workspace>/ai-talk-core/README.md) を編集するのが妥当。
- レビュー観点としては、再構成の軸を `Whisper の最小文字起こし` ではなく `音声で拾った指示を agent に handoff するローカルツール` へ明示的に寄せるべき。
- 章立ては `まず試す` -> `Web UI` -> `API / CLI` -> `Agent handoff` -> `Compatibility notes` -> `Architecture` の順がよい。これは [DESIGN_REVIEW.md](<workspace>/ai-talk-core/DESIGN_REVIEW.md#L67) の `3つの始め方` および [DESIGN_REVIEW.md](<workspace>/ai-talk-core/DESIGN_REVIEW.md#L81) の `読者別に分ける` 提案とも整合する。
- 実装担当が整理する際は、[SHARE_NOTE.md](<workspace>/ai-talk-core/SHARE_NOTE.md#L124) にある再編方針、すなわち `transcription core`, `session/state`, `driver`, `maintenance UI` の境界を README の説明順にも反映させるのが望ましい。
...
- 逆に [src/main.py](<workspace>/ai-talk-core/src/main.py), [src/web/app.py](<workspace>/ai-talk-core/src/web/app.py), [smoke_test.py](<workspace>/ai-talk-core/smoke_test.py) のように、複数責務を集約した箇所は、単一スレッド実装で局所最適の修正が入りやすく、整合崩れのリスクが高い。
...
- 特に [src/main.py](<workspace>/ai-talk-core/src/main.py), [src/web/app.py](<workspace>/ai-talk-core/src/web/app.py), [src/core/handoff_bridge.py](<workspace>/ai-talk-core/src/core/handoff_bridge.py), [src/runners/agent.py](<workspace>/ai-talk-core/src/runners/agent.py) は入口と境界が混ざりやすいため、先に契約を書いてから整理した方が安全。
...
- レビュー記録としては `REVIEW.md` に残す価値があるが、実効化には実装担当が後で [SHARE_NOTE.md](<workspace>/ai-talk-core/SHARE_NOTE.md) などの基準点へ `合意済み運用変更` として昇格させる必要がある。
...
- Low-Medium: 分担運用そのものは前進しているが、コードレビュー記録が最新実装に追いついていない。現行コードには [src/core/session.py](<workspace>/ai-talk-core/src/core/session.py#L35), [src/web/transcription_service.py](<workspace>/ai-talk-core/src/web/transcription_service.py#L83), [src/drivers/base.py](<workspace>/ai-talk-core/src/drivers/base.py#L29), [src/core/status_report.py](<workspace>/ai-talk-core/src/core/status_report.py#L1) が入り、README も `agent handoff` 主体に再構成されているが、[REVIEW.md](<workspace>/ai-talk-core/REVIEW.md) の最新レビュー群はまだ `src/main.py` / `src/web/app.py` の責務集中や旧 README 構成を前提にした所見が中心で、`108 tests OK` を含む今回の分割結果を評価しきれていない。運用としては `分担実装` は動いているが、`分担レビュー` はまだ同期不足。
- Low-Medium: [SHARE_NOTE.md](<workspace>/ai-talk-core/SHARE_NOTE.md#L17) 以降は current status, next tasks, collaboration request, handover notes が 1 ファイルに厚く積み上がっており、最新の統合済み事実と draft/補足が混ざりやすい。特に [SHARE_NOTE.md](<workspace>/ai-talk-core/SHARE_NOTE.md#L177) の worker 未提出メモのような統合担当向け注記まで同居していて、`いま main に何が入っているか` を一読で判断しづらい。
- Low: [OPERATIONS_DRAFT.md](<workspace>/ai-talk-core/OPERATIONS_DRAFT.md#L16) と [MODULE_REQUIREMENTS.md](<workspace>/ai-talk-core/MODULE_REQUIREMENTS.md#L17) は有効に機能しており、少なくとも `1スレッド = 1層 + 1目的`、`interface は薄く保つ`、`CLI / Web / API で core 状態管理を重複させない` という方針は、[src/main.py](<workspace>/ai-talk-core/src/main.py#L110), [src/web/app.py](<workspace>/ai-talk-core/src/web/app.py#L390), [src/runners/agent.py](<workspace>/ai-talk-core/src/runners/agent.py#L63) の実装にも反映され始めている。少なくとも今回確認した範囲では、分担の方向性は正しい。
- Low: 残る主なリスクは、コード分割より `smoke_test.py` と運用記録の集中である。[MODULE_REQUIREMENTS.md](<workspace>/ai-talk-core/MODULE_REQUIREMENTS.md#L117) でも当面は回帰防止優先としているため妥当ではあるが、レビュー/統合コストの中心は引き続きここに残る。
...
- 1. [REVIEW.md](<workspace>/ai-talk-core/REVIEW.md) に、今回の `session/service/driver` 分割を前提にした新しいレビュー節を作り、古い前提の所見と区別できるようにする。
- 2. [SHARE_NOTE.md](<workspace>/ai-talk-core/SHARE_NOTE.md) は `現在の統合済み事実` と `worker / integrator 向け補足` を分け、main の状態を一読で把握できるようにする。
...
- Low: `Worker_web_ui` の `Quick / Advanced / Debug` 分離は、`web/ui` 層の情報設計と表示密度の整理に留まる限り、`1層 + 1目的` に収まっている。対象は [src/web/app.py](<workspace>/ai-talk-core/src/web/app.py) と必要なら関連する最小の Web テストまでに留めるべきで、`transcription_service`, `handoff`, `driver` 側の契約変更まで広げないのが安全。
...
- Low: `Worker_drivers_handoff` の `DriverResult` 最小拡張は、backend 実行契約の明確化に留まる限り、`drivers/handoff` 層の単一目的として妥当。[src/drivers/base.py](<workspace>/ai-talk-core/src/drivers/base.py) と [src/runners/agent.py](<workspace>/ai-talk-core/src/runners/agent.py), [src/runners/ollama.py](<workspace>/ai-talk-core/src/runners/ollama.py) 程度で閉じるなら安全。
...
- Low-Medium: `Worker_web_ui` の統合結果 [src/web/app.py](<workspace>/ai-talk-core/src/web/app.py) は `Quick / Advanced / Debug` 分離に加えて、maintenance status, Result Center, copy action, latest handoff refresh action まで同時に入っており、`web/ui` 層の中では閉じているが、`1層 + 1目的` としてはやや広がり気味。大きく壊れてはいないが、次回以降は `モード分離` と `結果アクション追加` を別ターンに分けた方が統合事故を減らせる。
- Low-Medium: `Worker_drivers_handoff` の統合結果では、[src/drivers/base.py](<workspace>/ai-talk-core/src/drivers/base.py) に `validate_driver_command_available()` を追加しつつ、[src/runners/common.py](<workspace>/ai-talk-core/src/runners/common.py) 側にも `validate_runner_command_available()` が残っている。現時点では互換 alias として動くが、command availability の責務が driver 層と runner 層に二重化しており、将来ここがずれると境界崩れの温床になる。
- Low: `DriverResult` への `command_name`, `succeeded`, `has_output` 追加自体は小さく、[src/drivers/base.py](<workspace>/ai-talk-core/src/drivers/base.py), [src/runners/agent.py](<workspace>/ai-talk-core/src/runners/agent.py), [src/runners/ollama.py](<workspace>/ai-talk-core/src/runners/ollama.py) で閉じており妥当。ただし現時点では caller 側で十分に活用されておらず、ここから CLI 表示や Web UI 状態表示まで同時に広げると `drivers/handoff` を超えてにじみやすい。
...
## Review 2026-03-22 integrator message ownership check

### Findings

- Medium: [MEMORY.md](<workspace>/ai-talk-core/MEMORY.md#L33) [MEMORY.md](<workspace>/ai-talk-core/MEMORY.md#L35) [SHARE_NOTE.md](<workspace>/ai-talk-core/SHARE_NOTE.md#L15) [SHARE_NOTE.md](<workspace>/ai-talk-core/SHARE_NOTE.md#L22) では、`SHARE_NOTE.md` を `採用済みの現在地` だけでなく `統合担当の最新指示の正本` としても使う前提が入っている。`worker/web-ui` / `worker/drivers-handoff` への個別指示自体は妥当だが、これを `SHARE_NOTE.md` の正本役割へ昇格すると、統合済み事実と一時的な作業指示が同居し、`未合意の提案や途中経過を直接書かない` という運用と緊張する。現状のリスクは code の層越えより、記録の責務越えにある。
- Low-Medium: [OPERATIONS_DRAFT.md](<workspace>/ai-talk-core/OPERATIONS_DRAFT.md#L606) の `worker/web-ui` 保留メモは、`Review request draft` のテンプレート節に混ざっており、再利用するための運用テンプレートと、いま発生している ownership 衝突の事実が同じ場所に置かれている。これは `レビュー依頼フォーマット` の責務を超えており、次回以降に stale な実運用メモとして残りやすい。こうした個別案件は `LOG.md` か、明示的に期限付きの指示欄へ寄せた方が安全。
- Low: 境界そのものについては、今回見えた指示内容は概ね守れている。[SHARE_NOTE.md](<workspace>/ai-talk-core/SHARE_NOTE.md#L24) は `worker/web-ui` を `src/web/app.py` の表示整理へ限定し、[SHARE_NOTE.md](<workspace>/ai-talk-core/SHARE_NOTE.md#L33) は `worker/drivers-handoff` を `src/drivers/*`, `src/runners/*` の最小範囲へ限定している。少なくとも記述上は `core/session`, `drivers/handoff`, `web/ui` の分離方針と整合している。

### Open questions / assumptions

- 今回のレビュー対象は、コード本体の新規差分ではなく、分担運用を支える `MEMORY.md`, `OPERATIONS_DRAFT.md`, `SHARE_NOTE.md` の更新とした。
- `SHARE_NOTE.md` に `## Integrator messages` を置く案自体を否定しているわけではなく、`採用済み事実の共有` と `一時的な作業指示` を同じ正本へ混ぜる点を懸念している。

### Recommended next actions

- 1. `SHARE_NOTE.md` を使い続けるなら、`## Integrator messages` を `採用済み現在地` とは別の一時運用面であると明記し、履歴を持たない更新前提に固定する。
- 2. 個別の ownership 衝突や保留判断は、`OPERATIONS_DRAFT.md` のテンプレート節へ混ぜず、`LOG.md` または専用の一時指示欄へ寄せる。
- 3. レビュー観点としては、今後もしばらく `code の責務分離` と同じ強さで `記録の責務分離` も監視対象に含めるのがよい。
