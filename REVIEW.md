...
- Medium: [MEMORY.md](/home/hiromu/projects/ai_core/MEMORY.md#L33) [MEMORY.md](/home/hiromu/projects/ai_core/MEMORY.md#L35) [SHARE_NOTE.md](/home/hiromu/projects/ai_core/SHARE_NOTE.md#L15) [SHARE_NOTE.md](/home/hiromu/projects/ai_core/SHARE_NOTE.md#L22) では、`SHARE_NOTE.md` を `採用済みの現在地` だけでなく `統合担当の最新指示の正本` としても使う前提が入っている。`worker/web-ui` / `worker/drivers-handoff` への個別指示自体は妥当だが、これを `SHARE_NOTE.md` の正本役割へ昇格すると、統合済み事実と一時的な作業指示が同居し、`未合意の提案や途中経過を直接書かない` という運用と緊張する。現状のリスクは code の層越えより、記録の責務越えにある。
- Low-Medium: [OPERATIONS_DRAFT.md](/home/hiromu/projects/ai_core/OPERATIONS_DRAFT.md#L606) の `worker/web-ui` 保留メモは、`Review request draft` のテンプレート節に混ざっており、再利用するための運用テンプレートと、いま発生している ownership 衝突の事実が同じ場所に置かれている。これは `レビュー依頼フォーマット` の責務を超えており、次回以降に stale な実運用メモとして残りやすい。こうした個別案件は `LOG.md` か、明示的に期限付きの指示欄へ寄せた方が安全。
